...
assetRouter.route("/").post(
  ...
  async function (req, res, next) {
    try {
      ...
      // console.log("image :", req.files.thumbnail);
      if (
      ...
      const fileContent = fs.readFileSync(req.files.thumbnail[0].path);
...
assetRouter
  .route("/collections/:collectionType")
  .get(
    auth.verifyToken,
    verifyUser,
    checkIsInRole("admin"),
    ...
    async function (req, res, next) {
      try {
        ...
        console.log("CollectionrRsult = ", collectionResult);
        console.log("collections: ", collectionResult.length);
...
assetRouter
  .route("/collections/pending-verification/:collectionType/:start/:end")
  .get(
    auth.verifyToken,
    verifyUser,
    checkIsInRole("admin"),
