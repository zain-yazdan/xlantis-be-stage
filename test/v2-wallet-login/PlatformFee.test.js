...
const PlatformFeeModel = require("../../models/v2-wallet-login/PlatformFeeModel");
...
let requestBody, JWT, platformFeeId, superAdminJWT;
describe(`Test route: '/platform-fee/admin' [POST]`, () => {
  ...
  it("should validate missing request body parameter", (done) => {
    chai
      .request(server)
      .post("/v2-wallet-login/platform-fee/admin")
...
describe(`Test route: '/platform-fee/super-admin/accept' [PATCH]`, () => {
  ...
  it("should validate missing request body parameter", (done) => {
    chai
      .request(server)
      .patch("/v2-wallet-login/platform-fee/super-admin/accept")
...
describe(`Test route: '/platform-fee/super-admin/reject' [PATCH]`, () => {
  it("should validate missing request body parameter", (done) => {
    chai
      .request(server)
      .patch("/v2-wallet-login/platform-fee/super-admin/reject")
      .auth(superAdminJWT, { type: "bearer" })
      .end((err, res) => {
        expect(res).to.have.status(400);
        expect(res.body.success).to.be.eq(false);
...
describe(`Test route: '/platform-fee/admin/status' [GET]`, () => {
  it("should validate missing request body parameter", (done) => {
    chai
      .request(server)
      .get("/v2-wallet-login/platform-fee/admin/status")
      .auth(superAdminJWT, { type: "bearer" })
      .end((err, res) => {
        expect(res).to.have.status(400);
        expect(res.body.success).to.be.eq(false);
        expect(res.body.message).to.be.eq(
          "Platform Fee Status not found in query params"
        );
        done();
      });
  });
  ...
  it("should validate empty request body parameter", (done) => {
    chai
      ...
      .end((err, res) => {
        ...
        expect(res.body.success).to.be.eq(false);
        expect(res.body.message).to.be.eq(
          "Platform Fee Status was empty in query params"
        );
        done();
      });
  });
