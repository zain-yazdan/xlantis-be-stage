...
describe(`Test route: '/platform-fee/super-admin/accept' [PATCH]`, () => {
  ...
  it("should validate missing request body parameter", (done) => {
    chai
      .request(server)
      .patch("/platform-fee/super-admin/accept")
...
describe(`Test route: '/platform-fee/super-admin/reject' [PATCH]`, () => {
  it("should validate missing request body parameter", (done) => {
    chai
      .request(server)
      .patch("/platform-fee/super-admin/reject")
      .auth(superAdminJWT, { type: "bearer" })
      .end((err, res) => {
        expect(res).to.have.status(400);
        expect(res.body.success).to.be.eq(false);
  ...
  it("should change the platform fee status from the database", (done) => {
    ...
    describe(`Test route: '/platform-fee/super-admin' [POST]`, () => {
        it ('should verify server is running', (done)=>{
            chai.request(server)
                .get('/is-live')
                .end((err, res) => {
        ...
        it ('should validate missing request body parameter', (done)=>{
            chai.request(server)
            .post('/platform-fee/super-admin')
            .auth(superAdminJWT, { type: 'bearer' })
            .end((err, res) => {
                expect(res).to.have.status(400);
                expect(res.body.success).to.be.eq(false);
                expect(res.body.message).to.be.eq('Platform fee not found in request body');
                done();
            });
        });
        ...
        it ('should validate empty request body parameter', (done)=>{
            ...
            .end((err, res) => {
                expect(res).to.have.status(400);
                expect(res.body.success).to.be.eq(false);
                expect(res.body.message).to.be.eq('Platform fee was empty in request body');
                done();
            });
        });
        ...
        //POSITIVE CASE
        it ('should set the platform fee for super admin.', (done)=>{
            requestBody = { platformFee: 25 };
            ...
            .end((err, res) => {
                expect(res).to.have.status(200);
                expect(res.body.success).to.be.eq(true);
                expect(res.body.message).to.be.eq('Platform fee set up successfully.');
                done();
            });
        });
    });
    ...
    describe(`Test route: '/platform-fee/admin/status' [GET]`, () => {        
        ...
        it ('should validate missing request body parameter', (done)=>{
            chai.request(server)
            .get('/platform-fee/admin/status')
            .auth(superAdminJWT, { type: 'bearer' })
            .end((err, res) => {
                expect(res).to.have.status(400);
                expect(res.body.success).to.be.eq(false);
                expect(res.body.message).to.be.eq('Platform fee Status not found in request body');
                done();
            });
        });
